...
outdir <- "results/figure4/stress_4h"
...
low <- read.csv(
  "results/deseq2/PFBA_0.01_vs_Control_4h.csv",
  check.names = FALSE
)

high <- read.csv(
  "results/deseq2/PFBA_1_vs_Control_4h.csv",
  check.names = FALSE
)
...
names(low2) <- c("gene_id","Low_LFC_4h","Low_padj_4h")
names(high2) <- c("gene_id","High_LFC_4h","High_padj_4h")
...
dat$Low_sig_4h  <- !is.na(dat$Low_padj_4h)  & dat$Low_padj_4h  < 0.05
dat$High_sig_4h <- !is.na(dat$High_padj_4h) & dat$High_padj_4h < 0.05

dat$response_4h <- "Neither significant"
dat$response_4h[ dat$High_sig_4h & !dat$Low_sig_4h] <- "1 ug/g only"
dat$response_4h[!dat$High_sig_4h &  dat$Low_sig_4h] <- "0.01 ug/g only"
dat$response_4h[ dat$High_sig_4h &  dat$Low_sig_4h] <- "Both doses"

cat("\n============================================================\n")
cat("FULL-CONTRAST 4 h STRESS RESPONSE\n")
cat("============================================================\n\n")

cat("Stress genes tested:", nrow(dat), "\n")
cat("Significant at 1 ug/g    :", sum(dat$High_sig_4h), "\n")
cat("Significant at 0.01 ug/g :", sum(dat$Low_sig_4h), "\n")
cat("Shared significant      :", sum(dat$High_sig_4h & dat$Low_sig_4h), "\n\n")

cat("Response classes:\n")
print(table(dat$response_4h))

# high-significant genes with actual low-dose estimates
highsig <- dat[dat$High_sig_4h, ]
highsig <- highsig[order(highsig$High_padj_4h), ]
...
print(
  highsig[, c(
    ...
    "High_LFC_4h","High_padj_4h",
    "Low_LFC_4h","Low_padj_4h",
    "Low_sig_4h","response_4h"
  )],
  row.names = FALSE
)

# low-significant genes, if any
lowsig <- dat[dat$Low_sig_4h, ]

cat("\n============================================================\n")
cat("0.01 ug/g STRESS GENES AT 4 h\n")
cat("============================================================\n\n")
...
write.csv(
  dat,
  file.path(outdir, "FA26_stress_fullcontrast_high_vs_low_4h.csv"),
  row.names = FALSE
)

write.csv(
  highsig,
  file.path(outdir, "FA26_high_stress_genes_with_low_effects_4h.csv"),
  row.names = FALSE
)

# scatter
plotdat <- dat[
  !is.na(dat$Low_LFC_4h) &
  !is.na(dat$High_LFC_4h),
]

plotdat$label <- ifelse(
  plotdat$High_sig_4h | plotdat$Low_sig_4h,
  plotdat$display_name,
  ""
)

p <- ggplot(plotdat, aes(Low_LFC_4h, High_LFC_4h)) +
  ...
  geom_point(aes(shape = response_4h), size = 3, alpha = 0.85) +
  labs(
    x = expression("0.01 " * mu * "g g"^{-1} * " PFBA, 4 h log"[2] * "FC"),
    y = expression("1 " * mu * "g g"^{-1} * " PFBA, 4 h log"[2] * "FC"),
    shape = "4 h significance",
    title = "Canonical stress-associated transcription at 4 h"
  ) +
...
ggsave(
  file.path(figdir, "Figure4_stress_4h_fullcontrast_exploratory.pdf"),
  p,
  width = 7.2,
  height = 6.3
)
...
ggsave(
  file.path(figdir, "Figure4_stress_4h_fullcontrast_exploratory.tiff"),
  p,
  width = 7.2,
  height = 6.3,
  dpi = 600,
